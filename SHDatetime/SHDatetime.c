...
static bool _calcFractFromParts(double miliseconds,double* ans,int *error);
static void _popDtWithFracSecs(SHDatetime* dt,FractSecs* fractSecs);
static bool _shouldAddLeapDay(long year,int month,int day);
...
static int _calcYearsBefore20thCent(long seconds,bool isLeapCent){
    if(seconds < 0) return -1;
    int years = 0;
    (void)isLeapCent;
    int leapExcess = seconds % SECONDS_PER_4_YEARS;
...
static long _calcYearsPastPerQuadricentennial(long seconds){
    if(seconds < 0) return -1;
    long years = 0;
    long leapExcess = 0;
    long leapCycles = 0;
...
static long _calcYearsPerQuadricentennial(long seconds){
    long years = 0;
    long leapExcess = 0;
    long leapCycles = 0;
    if(seconds > SECONDS_PER_400_YEARS){
...
bool tryDtToTimestamp(SHDatetime const *dt,double *ans,int *error){
    *error = NO_ERROR;
    ...
    isSuccess = tryCreateDateTime(dt->year,dt->month,dt->day,dt->hour,dt->minute
...
bool tryExtractTime(SHDatetime *dt,double *ans,int *error){
    *error = NO_ERROR;
    ...
    bool isSuccess = _calcFractFromParts(dt->milisecond,&fraction,error);
    if(!isSuccess) return false;
    isSuccess = tryCreateTime(dt->hour,dt->minute,dt->second,ans,error);
    return isSuccess;
}
...
int calcDayOfYear(SHDatetime *dt,int *error){
    if(!isValidSHDateTime(dt)) return setIndexErrorCode(GEN_ERROR,error);
...
int calcDayOfYearFromTimestamp(double timestamp,int timezoneOffset,int * error){
    SHDatetime dt;
    if(!tryTimestampToDt(timestamp,timezoneOffset,&dt,error)) return NOT_FOUND;
    return calcDayOfYear(&dt,error);
}
...
static bool _calcFractFromParts(double miliseconds,double* ans,int *error){
    *error = NO_ERROR;
    if(miliseconds > 1000) return setErrorCode(OUT_OF_RANGE,error);
